...
wt_path = os.path.join(DATA_SET_PATH, "yolo.weights")
...
for i in range(1, nb_conv + 1):
    ...
    if len(conv_layer.get_weights()) > 1:
        ...
        kernel = weight_reader.read_bytes(np.prod(conv_layer.get_weights()[0].shape))
        kernel = kernel.reshape(list(reversed(conv_layer.get_weights()[0].shape)))
        #转至https://blog.csdn.net/u012762410/article/details/78912667
        #2 表示shape[0]和shape[2]轴对换  3表示shape[1]和shape[3]轴对换  1 shape[2]和shape[1]轴对换  0 shape[3]和shape[0]对换
        kernel = kernel.transpose([2, 3, 1, 0])
        conv_layer.set_weights([kernel])
        print("handle conv_" + str(i) + " completed")

#由於在YOLOv2的模型中, 最後一層卷積層決定了最後的輸出, 讓我們重新來調整與訓練這一層的卷積層來讓預訓練的模型可以讓我們進行所需要的微調
layer   = model.layers[-4] # 找出最後一層的卷積層1*1卷积层
